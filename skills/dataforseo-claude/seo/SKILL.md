...
```bash
cat /paperclip/dataforseo-claude/.env 2>/dev/null
```
...
I'll save them securely to /paperclip/dataforseo-claude/.env (chmod 600) and run
...
2. Write them to `/paperclip/dataforseo-claude/.env` using this format:
...
```bash
chmod 600 /paperclip/dataforseo-claude/.env
```

4. Run a verification call (cheap — ~$0.001):

```bash
/paperclip/dataforseo-claude/scripts/keyword_research.py volume "test" 2>&1
```
...
2. Add credentials to `/paperclip/dataforseo-claude/.env`:
...
All scripts below live at `/paperclip/dataforseo-claude/scripts/` and have shebangs
...
```bash
/paperclip/dataforseo-claude/scripts/domain_overview.py overview --target <domain>
/paperclip/dataforseo-claude/scripts/backlinks.py summary --target <domain>
/paperclip/dataforseo-claude/scripts/domain_overview.py ranked --target <domain> --limit 25
```
...
3. Write it to `/paperclip/dataforseo-claude/output/<domain>-audit.json`.
4. Offer to generate the PDF: `/seo report-pdf <domain>`.

### `/seo keywords <seed>`

```bash
/paperclip/dataforseo-claude/scripts/keyword_research.py related "<seed>" --limit 200
/paperclip/dataforseo-claude/scripts/keyword_research.py suggestions "<seed>" --limit 100
```
...
```bash
/paperclip/dataforseo-claude/scripts/on_page_audit.py site --target <domain> --max-crawl-pages 100
```
...
```bash
/paperclip/dataforseo-claude/scripts/on_page_audit.py page --url <url>
```

### `/seo competitors <domain>`

```bash
/paperclip/dataforseo-claude/scripts/domain_overview.py competitors --target <domain> --limit 20
```
...
```bash
/paperclip/dataforseo-claude/scripts/domain_overview.py ranked --target <domain> --limit 200
```
...
```bash
/paperclip/dataforseo-claude/scripts/backlinks.py summary --target <domain>
/paperclip/dataforseo-claude/scripts/backlinks.py refdomains --target <domain> --limit 50
/paperclip/dataforseo-claude/scripts/backlinks.py anchors --target <domain> --limit 30
```
...
```bash
/paperclip/dataforseo-claude/scripts/serp_check.py rank --domain <domain> --keywords kw1 kw2 ...
```
...
```bash
/paperclip/dataforseo-claude/scripts/domain_overview.py content_gap --you <you> --competitors <comp>
```
...
- Save all intermediate JSON to `/paperclip/dataforseo-claude/output/<domain>-<command>.json`
